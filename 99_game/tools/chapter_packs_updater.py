...
结构：
    {
      "chapter01_新皮肤": {"portraits": ["陈默.沉重", ...], "scenes": ["长江大桥-栏杆", ...]},
      ...
    }

数据源由 chapter-publisher 提供（图查的 depicts 立绘 <char>.<variant> + has_layer 背景 <Scene.name>）。
幂等：覆盖该 stem 条目，保留其他章。无依赖（仅标准库）。

CLI: chapter_packs_updater.py <stem> [--portraits a,b] [--scenes x,y] [--packs <path>]
"""
import argparse
import json
import sys
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"OK: {args.stem} portraits={len(entry['portraits'])} "
        f"scenes={len(entry['scenes'])} -> {path}"
    )
    return 0
