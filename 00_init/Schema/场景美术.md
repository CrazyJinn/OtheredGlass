...
> **美术风格**：所有美术风格定义（画风、光线、渲染风格等）统一写入 [00_init/美术风格.md](../../00_init/美术风格.md)，不再存储为图节点。阵营配色表、通用风格元素亦见该文件。
...
| camp | 阵营 | string | 否 | 标签：骑士团/秘术协会/教会/废土/变异森林/灰雪地/无 |
| color_direction | 配色逻辑 | string | 否 | 自由文本：主辅点缀色与明暗逻辑 |
| art_element | 风格元素 | string | 否 | 标签（可多选）：粗犷/厚重/蒸汽朋克/做旧处理/黑暗氛围 |
