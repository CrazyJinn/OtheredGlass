...
> **设计元素标签**：数据节点（AppearanceStyle / CostumeStyle）及立绘（StandingIllustration）的**可枚举设计维度**以**标签**形式存储，字段值为分号 `;` 分隔的标签串。受控词表见 [55_manage/标签库.json](../../55_manage/标签库.json)，由管理后台标签选择组件编辑。无法枚举的维度（综合外貌、配色逻辑、具体款式等）仍保留自由文本字段。prompt 不再存节点，由 `char-prompt-assembler` 读 tags + 美术风格.md 派生为文件，节点存 `prompt_path`。
...
**status 枚举**：`0` 待生成 → `1` 提示词文件已生成 → `2` 图片生成完成
