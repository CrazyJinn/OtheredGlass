...
- **章节与图的关系**：每个运行时章节 JSON 对应图中的 `Chapter` 节点；章下挂若干 `Section` 节点（`has_section` 边），每节锚定自己的 `outline_path` / `script_path`（创作区节级 YAML）。编排信息（标题/序号/分节/场景顺序/分支骨架）入图作设计层，逐句对话不入图——运行时只读本 JSON，图仅供生成与分析（见 [Schema/剧情.md](Schema/剧情.md)）。
...
|------|------|------|--------|------------------|
| 创作区提纲（节级） | `25_剧本/chapter<NN>_<章概述>/sec<MM>_<节概述>/outline.yaml` | YAML | `chapter-outliner` | 否（含 `authoring` 人读字段） |
...
`chapter-outliner` 产出**节级**提纲，落盘 `25_剧本/chapter<NN>_<章概述>/sec<MM>_<节概述>/outline.yaml`（YAML，人读）。提纲 = **拓扑骨架（schema 子集字段）+ `authoring` 人读引导块**。
