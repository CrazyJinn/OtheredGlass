...
- **创作区与运行时区分离**：`screenwriter` skill 产出的剧本先落 **`25_剧本/`**（创作/审阅区，`Chapter.script_path` 指向此处，dashboard 在此预览/审批）；审阅通过（`status=11`）且所需立绘就绪后，由 `chapter-publisher` skill 把剧本 + 涉及的立绘/背景资源打包发布到 **`99_game/data/chapters/`** + `99_game/assets/`（Godot 运行时消费区）。两区解耦：改 `25_剧本/` 不影响已发布版本。
