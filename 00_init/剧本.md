...
- **一个章节 = 创作区一个 `.yaml` 文件 + 运行时一个 `.json` 文件**，同名主体（如 `chapter01_新皮肤`），文件名即章节标识。创作区 `.yaml` 由 `chapter-dialoguer` 产出，`chapter-publisher` 发布时转换为 `99_game/data/chapters/<同名>.json`。章节划分建议对齐 [大纲.md](../大纲.md) 的叙事段落。
- 章节内含该章**完整对话**。
...
|------|------|------|--------|------------------|
| 创作区提纲 | `25_剧本/chapter<NN>_<概述>.outline.yaml` | YAML | `chapter-outliner` | 否（含 `authoring` 人读字段） |
| 创作区定稿 | `25_剧本/chapter<NN>_<概述>.yaml` | YAML | `chapter-dialoguer` | 是（schema 子集 1:1） |
| 运行时定稿 | `99_game/data/chapters/chapter<NN>_<概述>.json` | JSON | `chapter-publisher`（转换） | 是（转换后复校） |

- **创作区 YAML**：人读友好（缩进、`#` 注释）；提纲含 `authoring` 引导块，定稿是 schema 子集 1:1。
- **运行时 JSON**：Godot `JSON.parse` 直接消费；由 `chapter-publisher` 用 [yaml_to_chapter_json.py](../99_game/tools/yaml_to_chapter_json.py) 从定稿 YAML 转换（单向、幂等、无信息损失）。
- **定稿 YAML ↔ 运行时 JSON 字段完全一致**，仅语法不同；同名主体，跨章节 `file` 引用用不带后缀的 stem。

### YAML 写作安全规则（所有创作 skill 遵守）
...
`chapter-outliner` 产出章节提纲，落盘 `25_剧本/chapter<NN>_<概述>.outline.yaml`（YAML，人读）。提纲 = **拓扑骨架（schema 子集字段）+ `authoring` 人读引导块**。

**拓扑骨架**（与定稿同结构，`chapter-dialoguer` 必须原样搬到定稿）：
...
| 字段 | 位置 | 含义 |
|------|------|------|
| `direction` | 顶层 authoring | 一段话讲清本章剧情发展方向（核心人读字段） |
| `emotion_arc` | 顶层 authoring | 情感弧线起点 → 终点 + 中途转折 |
| `constraints` | 顶层 authoring | 给 dialoguer 的硬约束清单 |
| `purpose` | 每 scene 的 authoring | 这场戏的戏剧职责 |
| `beats` | 每 scene 的 authoring | 节拍走向（自然语言列表，**禁写台词**）—— 解决事件粒度粗 |
| `motif_anchors` | 每 scene 的 authoring | 母题 / 象征物锚点 |
| `transition` | 每 scene 的 authoring | 场景衔接方式说明 |

提纲确定「章节分几段、每段场景/时间/bgm、choice 分叉与汇合、ending 位置」的骨架；`chapter-dialoguer` 读提纲文件，以 `authoring.beats` 为节拍依据，在**保持拓扑不变**的前提下填充逐句 `say` / `narrate` / `show` 等 lines，产出完整定稿 `.yaml`（同名主体，去掉 `.outline` 中缀）。**`authoring` 块不搬进定稿**。
...
`chapter01_新皮肤.json`（运行时 JSON 形态节选，含场景段切换、分支、结局）。创作区 YAML 定稿结构与之 1:1，仅语法差异；提纲 YAML 额外含 `authoring` 块：
...
- 章节文件名：`chapter<编号>_<概述>`（创作区 `.yaml` / 运行时 `.json`，如 `chapter01_新皮肤.yaml` → `chapter01_新皮肤.json`）。
