...
SET n.status = $status, n.description = $desc
...
SET n.name = $name, n.description = $desc, n.status = 0
...
// wears: Event → CostumeStyle（sync=false）
MATCH (a:Event {id: $from_id}), (b:CostumeStyle {id: $to_id})
MERGE (a)-[r:wears]->(b)
SET r.sync = false
...
### 为指定着装创建 IllusDesign + StandingIllustration
