---
name: infra-neo4j-helper
description: |
  自然语言 + Schema → Cypher → 执行 → 结构化返回。支持多语句、增删改查混杂，
  Schema 动态读取适配不同图模型。在需要操作 Neo4j 图数据库、将自然语言转为 Cypher、
  其他 skill 读写图数据、或用户提到"查图/图数据库/cypher/导入数据"时使用。
  前置：Neo4j 服务已启动，neo4j 包已安装（pip install neo4j）。
...
密码从**工作目录**向上搜索 `settings.json`，读取 `neo4j_password` 字段。

优先级从高到低：

1. 命令行参数 `--uri` / `--user` / `--password`
2. 工作目录 `settings.json` 中的 `neo4j_password` 字段
3. 默认值：`bolt://localhost:7687` / `neo4j` / `12345678`
