...
**通过 neo4j-helper skill 以自然语言查询**每个角色：

> "查询编号为 <角色编号> 的角色节点，返回 design_prompt_path、stand_painting_prompt_path 和 status"

获取以下信息：
- `status`：前置检查，status 必须 >= 2（美术提示词已完成）。status < 2 时跳过并记录到 unable_to_process
