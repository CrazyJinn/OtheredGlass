...
**服装（garment）**：服装信息的**唯一数据源**，标签合成组合 = 材质 + 颜色 + 类型。受控词表见 `55_dashboard/config/标签库.json`，dashboard 按子维度选择后自动合成，如 `棉白衬衫`；多件用分号分隔，如 `棉白衬衫;蕾丝黑内衣` → `CostumeStyle.garment`

> 标签未覆盖的款式细节（领型、滚边、层次、剪裁、质感氛围等），用标签库的**自定义输入**直接补充到对应标签值，不再另设自由文本字段。
