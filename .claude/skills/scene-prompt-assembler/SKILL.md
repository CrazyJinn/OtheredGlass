...
- `camp`（阵营）→ 查阵营配色表（主色调/光效/氛围关键词）
- `art_element`（风格元素标签，分号分隔）→ 末尾风格词
...
- **阵营配色查表**：按 `camp` 取主色调/光效/氛围，融入光影与环境段
- **场景无角色**：末尾须含「无角色」（场景图不出现角色立绘，与角色 #00FF00 背景互斥）
- **硬约束收尾**：末尾须含「油画风格，手绘质感，游戏美术」
- **只提取不创作**：内容来自 data 参数（Scene 字段）和 `00_init/美术风格.md`，不臆造

## 输出流程

1. 解析 data，提取 `scene.*` 字段、`location.name`、`node.id`，以及调用方声明的 `output_path`
2. 从 `00_init/美术风格.md` 读取「场景美术风格」块（基础定位、渲染、阵营配色表、通用风格元素、提示词硬约束）
...
```json
{
  "scene": {
    ...
    "atmosphere": "...", "composition": "...", "lighting": "...", "camp": "...",
...
4. **光影**：展开 `lighting`（主光源方向+色温+环境光）；按 `camp` 查配色表补主色调与光效
5. **风格**：`color_direction`（如有）+ `art_element` 风格元素标签展开，末尾「油画风格，手绘质感，游戏美术，无角色」
...
- [场景提示词模板](references/template-场景提示词.md) — 用途→主体→环境→光影→风格 结构、各 scene_type 模板、阵营配色表、风格元素表、检查清单（模式A）
