...
- action: `ADD_EDGE occurred_at(evt_id, loc_???) {detail: '?'}`
...
```cypher
MATCH (e:Event) RETURN e.编号 AS id, e.标题 AS title, e.时间 AS time
```
...
- action: "如果属于某阵营: ADD_EDGE BELONGS_TO(char_id, faction_???) {role: '?'}"
...
- action: `ADD_EDGE evt_relation(evt_???, id) {type: '因果|先后|包含', detail: '?'}`
  ...
  "suggestions": [
    {
      ...
      "action": "ADD_EDGE relation(char_005, char_007) {type: '?', detail: '?'}",
      "char1_id": "char_005",
      "char2_id": "char_007"
    }
  ],
