...
调用 prompt-assembler skill（Mode A DesignSheet 模式）：

使用 Skill 工具调用 `char-design:prompt-assembler`，传入参数 `<node_id> DesignSheet`。

prompt-assembler 将：
- 读取 AppearanceStyle + CostumeStyle 属性
...
调用 image-generator skill（DesignSheet 文生图模式）：
