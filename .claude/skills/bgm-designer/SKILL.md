...
name: bgm-designer
description: |
  推进 BgmTrack 图节点的描述段（人工生成链的 skill 侧半程）：查 Scene/BgmTrack 状态 → 依场景情绪与章节设计简报生成音乐描述文字 prompt（乐器/节奏/情绪/时长/结构，供 Suno 类外部工具）→ 写图 prompt/description + status=1 → **把文字产出给用户**并明示归档路径 13_音乐/<name>.wav。
  音频由用户手动生成并放入预制路径（手工放入文件夹即合格，无审批）；再次触发本 skill（或 dashboard 编辑器）检测文件存在置 status=2。由用户直接触发（不进 plot-design 编排，模式同 chapter-publisher）。
...
> 关联：`Scene -[:has_bgm]-> BgmTrack` 1:1（sync=false，BGM 独立资产）。缺口节点由 `chapter-dialoguer` 兜底建（status=0 + has_bgm 边）；发布时 `chapter-publisher` 把 status=2 的 track 拷 `99_game/assets/bgm/` + manifest.bgm 图驱动更新 + 注入章 JSON scene-block.bgm。
...
- **无节点**：该 Scene 无关联 BgmTrack——提示先跑 `chapter-dialoguer`（定稿时兜底建）或用 dashboard 手建；不自行建（BgmTrack 需求源于剧本 scene 行）。
...
依据：Scene 的 atmosphere/time_of_day/description + 所属章节的设计简报情感弧（若可定位：`MATCH (ch:Chapter)-[:has_section]->(:Section)-[:contains]->(s:Scene)` 找章，Read `ch.brief_path`）+ 提纲的「BGM 倾向」散文（若有）。
