...
4. 对需**新建**的 Scene，生成 snowflake id 并按下方字段表填写内容：
   ...
   | 阵营 | camp | 骑士团/秘术协会/教会/废土/变异森林/灰雪地/无 | 从世界设定推导；都市/现实题材填「无」 |
   | 配色逻辑 | color_direction | `主色咖啡棕，辅色奶白，点缀暖橙` | 自由文本：主辅点缀色与明暗逻辑 |
   | 风格元素 | art_element | 粗犷/厚重/蒸汽朋克/做旧处理/黑暗氛围 | 标签可多选，按场景类型查风格元素表 |
   | 概要 | description | `咖啡店的点餐区域` | 1-2 句 |

   **内容生成规则**：
   1. 阵营配色参考 [00_init/美术风格.md](00_init/美术风格.md) 的阵营配色表。
   2. 构图（composition）须明确远/中/近景三段，是下游 SceneLayer 背景提示词的主体来源。
   3. 光影（lighting）须含主光源方向+色温+环境光，是提示词光影段来源。
   4. 都市/现实题材场景 camp 填「无」，配色与光效由 color_direction/lighting 自由定调。
...
SET s.name = '<Location名-区域>',
    ...
    s.camp = '...', s.color_direction = '...', s.art_element = '...',
    s.description = '...',
    s.status = 1;
```
...
- [场景美术风格](00_init/美术风格.md) — 阵营配色表、通用风格元素
