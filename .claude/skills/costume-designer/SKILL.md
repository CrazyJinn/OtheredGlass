...
```cypher
// 角色基本信息
MATCH (ch:Character {id: 'char_NNN'}) RETURN ch;
...
// 已有 CostumeStyle
MATCH (ch:Character {id: 'char_NNN'})-[:has_costume]->(cos:CostumeStyle)
RETURN cos.id, cos.name, cos.default_outfit, cos.accessories, cos.approve;

// 已有 wears 边（哪些事件已有着装）
MATCH (e:Event)-[r:wears]->(cos:CostumeStyle)
MATCH (ch:Character {id: 'char_NNN'})-[:involved]->(e)
RETURN e.id AS event_id, cos.id AS costume_id, cos.name AS costume_name;

// AppearanceStyle（配色参考）
MATCH (ch:Character {id: 'char_NNN'})-[:has_appearance]->(app:AppearanceStyle)
RETURN app.color_direction, app.shape_language, app.visual_tone;
```
...
- [角色美术设定模板](${CLAUDE_SKILL_DIR}/../concept-designer/references/template-角色美术设定.md) — 材质方向、体态气质、配饰规则
