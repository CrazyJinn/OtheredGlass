...
description: |
  推进 Chapter 图节点的结构段：解析/创建 Chapter → 校验 Scene 前驱 → 把 N 个 Scene 统合进章节（contains 边）→ 写 status=1（结构就绪，待结构审）。
  纯图操作，无文件产出。在需要建立章节结构或重做章节结构时使用。
...
> **status=-1 = 作废重做**：当 Chapter 被 sync 级联或手动重置为 `status=-1` 时，即使 contains 边已存在，也**必须重新统合 Scene 并覆盖**（重走 0→1）。重做时先删除旧 contains 边再重建（contains 是本章权威，order 可能变）。禁止因 contains 已存在而跳过。

# 章节结构（Chapter 结构段 · status 0→1）

剧情创作三段式（`chapter-structurer` → 结构审 → `chapter-outliner` → `chapter-dialoguer` → 定稿审）的**第一段**。本 skill 只建立章节骨架：创建/补全 Chapter 节点的编排属性 + 用 `contains` 边把本章用到的 N 个 Scene 统合进来，推进到 `status=1`（结构就绪）。**不创作对话、不产文件**。
...
> 本 skill 是 Chapter 结构段 status 的写入点。纯图操作，不产出文件、不调子 skill。
...
据 `summary` 的叙事范围，从候选 Scene 中选定本章统合的 N 个 Scene，并排定 `order`（按本章剧情首次出现顺序，从 0 起）。这是结构编排决策（不涉及对话创作）：确定「本章分几段、每段在哪个 Scene、先后顺序」。
...
最后汇总：统合的 Scene 列表（含 order）、Chapter `status=1`。
...
- 下游：[chapter-outliner](../chapter-outliner/SKILL.md)（结构批 status=11 后产出提纲）
