...
description: |
  ...
  按角色 VoiceProfile（图 Character→has_voice_profile→VoiceProfile，status=1）：Qwen VoiceDesign 出 ref_audio（.venv-qwen，voice_clone_runner.py ensure-ref）→ CosyVoice3 inference_instruct2（.venv-cosyvoice，cosyvoice_runner.py publish，按 say.emotion 映射 instruct 控情绪）逐句克隆 → wav 落 99_game/assets/voices/<char>-<stem>-<scene_id>-<line_idx>.wav；voice_bundler.py inject 给 chapter JSON 每个 say 注入 voice 字段 + manifest 写 voices 段 + chapter_packs.voices（三处同源对齐，键由 make_voice_key 单一生成）。
...
按角色 `VoiceProfile`（图 `Character→has_voice_profile→VoiceProfile`）载入可复用 clone prompt，用 [voice_clone_runner.py](../../../.claude/scripts/voice/voice_clone_runner.py) 逐句克隆 wav → `99_game/assets/voices/`，再用 [voice_bundler.py](../../../.claude/scripts/voice/voice_bundler.py) 给 chapter JSON 每个 say 注入 `voice` 字段、更新 `manifest.voices` 与 `chapter_packs.voices`。

**voice 与 portrait 同构**（字符串键 → manifest → res:// 路径），三处对齐契约同立绘 guid 整键：wav 文件名 == `manifest.voices` 键 == chapter JSON `say.voice`，单一来源 [voice_bundler.make_voice_key](../../../.claude/scripts/voice/voice_bundler.py)：`<char>-<stem>-<scene_id>-<line_idx>`。
...
python .claude/scripts/voice/voice_bundler.py tasks '99_game/data/chapters/<stem>.json' \
...
> 项目内 venv（见 [15_声音/README.md](../../../15_声音/README.md)）：`.venv-qwen`（Python 3.14，Qwen VoiceDesign 出 ref）+ `.venv-cosyvoice`（Python 3.10，CosyVoice3 clone）。
> CosyVoice 按 `say.emotion` 映射 instruct 控情绪（emotion → instruct 映射见 [.claude/scripts/voice/emotion_instruct.json](../../../.claude/scripts/voice/emotion_instruct.json)）。

#### 3a. Qwen VoiceDesign 出 ref_audio（.venv-qwen）

复用已有 ref_audio 或首次 VoiceDesign 合成（按 VoiceProfile.instruct）：

```bash
.venv-qwen/Scripts/python.exe .claude/scripts/voice/voice_clone_runner.py ensure-ref \
...
```bash
.venv-cosyvoice/Scripts/python.exe .claude/scripts/voice/cosyvoice_runner.py publish \
...
python .claude/scripts/voice/voice_bundler.py inject '99_game/data/chapters/<stem>.json'
...
python .claude/scripts/voice/voice_bundler.py list '99_game/data/chapters/<stem>.json' > /tmp/voices.txt
...
"
# manifest 改用 ogg 扩展
python .claude/scripts/voice/voice_bundler.py manifest '99_game/data/chapters/<stem>.json' --ext ogg
```
...
- voice 键生成与绑定：[voice_bundler.py](../../../.claude/scripts/voice/voice_bundler.py)（make_voice_key / inject / manifest / list / tasks / tasks-from-section / inject-yaml）
- 批量克隆执行：[voice_clone_runner.py](../../../.claude/scripts/voice/voice_clone_runner.py)（ensure-ref 出 ref_audio）+ [cosyvoice_runner.py](../../../.claude/scripts/voice/cosyvoice_runner.py)（publish 逐句 clone）
