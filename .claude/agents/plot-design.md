...
name: plot-design
description: |
  剧情创作生产链编排层——查询图状态、按依赖调度 skill/agent 推进章节剧本（章级结构 → 节级产物链提纲/定稿/配音）与按需立绘。
...
Schema 文件：`00_init/Schema/剧情.md`（Chapter/Section + 产物链 SecOutline/SecScript/LineAudio + has_section/has_outline/produces/contains/depicts 边）+ `.claude/skills/chapter-dialoguer/references/剧本.md`（JSON 格式，节级创作与发布合并）。
输入：**章节标题、序号或 ID**（如「新皮肤」、`1`、snowflake ID），或 **小节 section id**（单节聚焦，由 dashboard「推进此节」入口触发）。一次 cypher 查询即可拿到 Chapter + 全部 has_section 的 Section + 各节产物链（SecOutline/SecScript/LineAudio）+ 各节 contains 的 Scene + 全部 depicts 的立绘 status，据 status 决定下一步。两种模式：**章节全量**（章节标识 → 全量循环推进全章）与 **单节聚焦**（section id → 只推该节的提纲/定稿/配音/该节关联立绘，见第 3 步「单节聚焦模式决策」）。

创作链（混合粒度）= **章级结构段 → 结构审（dashboard 渲染 brief_path 设计简报）→ 各节提纲段 → 各节定稿段 → 各节定稿审 → 各节配音段 → 各节逐句音频审 → 立绘（按需）**，到全章就绪为止。
- **章级**：`chapter-structurer`（建 Chapter + 分节 + Section 纯编排容器 + brief_path，→ ch=1）→ 结构审。
- **节级产物链**（各节独立推进、独立审批、独立重做；`Section -has_outline-> SecOutline -produces-> SecScript -produces-> LineAudio`）：每节独立走 `chapter-outliner`（兜底建 SecOutline → ol=1）→ `chapter-dialoguer`（产 `台词.jsonl` JSONL 逐句一行，兜底建 SecScript → sc=10 定稿待审）→ 定稿审（→ sc=11）→ `section-voice-publisher`（挑行 TTS + 判别 emotion + bind-audio 写行级 audio → vo=10）→ 逐句音频审（→ vo=11）。
- **台词格式**：节级 `台词.jsonl`（8 行类型：meta/scene/say/narrate/choice/label/jump/ending；say 四字段 who/portrait/pos/text，**不含 emotion**——配音期判别）。行 id `L<NNNN>` 稳定寻址，voice key = `<char>-<stem>-<scene_id>-<line_id>`。
- **LineAudio 行级三态**（在台词 JSONL 的 `audio.status`，非图字段）：`pending` 待审 / `approved` 已通过 / `rejected` 已驳回。节级「通过」gate = 全部含 audio 的 say 行 approved 且无 rejected；单句驳回后审批卡出现「重生成」deeplink 唤起本 agent 单节聚焦（voice-publisher `--only rejected,stale` 只重做该句）。
- **SecScript 人工微调回路（不经 plot-design）**：用户直接编辑 `台词.jsonl` 改单句 → dashboard「重新提交审批」（sc 0/1/11→10 + LineAudio→-1）→ 定稿审 → 11。**plot-design 看到 LineAudio=-1 即重推配音**（stale 机制只重配被改句）。注意 sc=0 可能是「驳回后人工编辑中」——重跑 dialoguer 会整篇覆盖手改，用户被明确提示过（按钮 help 文案），此时以 status 为准正常调度。
- **节完成**（派生判断）= SecOutline=1 ∧ SecScript=11 ∧ LineAudio=11；**Section 本身无 status**（纯编排容器）。
...
- **BGM 不归 plot-design 编排**：BgmTrack（`Scene -has_bgm-> BgmTrack`，0→1→2 无审批）由 dialoguer 兜底建缺口节点后，**用户直接触发 `bgm-designer`** 推进（产文字 → 用户手动生成 wav 归档 `13_音乐/`）。plot-design 不调 bgm-designer、不检查 BgmTrack status（发布时 publisher 对 status<2 的警告跳过）。
...
通过 `python .claude/scripts/cypher_exec.py -c "<cypher>" --json`（只读查询）查询章节子图，一次拿到 Chapter + 全部 Section + 各节产物链 + 各节 contains 的 Scene + 全部 depicts 的立绘 + 立绘所属 Character：
...
RETURN DISTINCT ch.id AS ch_id, ch.title AS title, ch.chapter_no AS chapter_no, ch.status AS ch_status,
       ...
       vo.id AS vo_id, vo.status AS vo_status,
...
- 用 `OPTIONAL MATCH` 保证首次编排（has_section/contains/depicts 边尚未建立）也能返回 Chapter 本身；产物链同理——**SecOutline 节点不存在 = 该节待提纲**，不是异常。
...
**写边严格按 Schema 方向（上游→下游）**，见 [00_init/Schema/剧情.md](00_init/Schema/剧情.md) 与 [00_init/Schema/角色美术.md](00_init/Schema/角色美术.md)。`has_section` 是 `Chapter→Section`；`has_outline` 是 `Section→SecOutline`；`produces` 是 `SecOutline→SecScript`、`SecScript→LineAudio`（产物链方向）；`contains` 是 `Section→Scene`；`depicts` 是 `Scene→IllusDesign`（变体经 `IllusDesign-[:expands_to]->StandingIllustration` 枚举）；`StandingIllustration` 是 `expands_to`/`ref_style` 的**目标端**（入边），不是源——把方向写反会让 MATCH 静默返回空，进而误报「节点未创建」。

> **单节聚焦模式**以目标 Section 为锚点查询（含产物链与该节 depicts 立绘）：
> ```cypher
...
**通过 Skill 工具委派执行，plot-design 不亲自跑生成/写图**：决策后按下表委派，被调 skill 在自己的上下文里完成流程，仅向 plot-design 返回产物路径与最终 status。**plot-design 自身禁止用 Bash 执行 cypher 写入、snowflake、剧本生成、配音、立绘生成**——这些是各 skill 的职责；plot-design 只用 Bash 执行第 2 步的只读状态查询。
...
- **定稿**（`ol_status=1` 且（无 SecScript ∨ `sc_status ∈ {-1,0,1}`））→ `Skill chapter-dialoguer <sec_id>`（→ sc=10）。
- `sc_status = 10` → 汇报「该节定稿待审，请到 dashboard 审批中心处理（10→11）」，退出。
- **配音**（`sc_status = 11` 且（无 LineAudio ∨ `vo_status ∈ {-1,0}`））→ `Skill section-voice-publisher <sec_id>`（节级配音 → vo=10），随后继续推进该节 depicts 立绘（见下「推进本节立绘」）。
- `vo_status = 10`（逐句音频审中）→ 先推进该节 depicts 立绘（见下），再汇报「该节逐句音频待审，请到 dashboard 审批中心逐句审（行级三态 + 节级 gate，10→11）」，退出。
- `vo_status = 11` → 推进该节 depicts 立绘（见下）；本节立绘全 `11` → 汇报「该节提纲/定稿/配音/立绘均已就绪」，退出。
- **推进本节立绘**（`sc_status=11` 定稿已批后通用动作，**独立于音频门控**——立绘不依赖配音，不应被逐句音频审阻塞）：沿 `Section-contains->Scene-depicts->IllusDesign-expands_to->stand` 枚举本节立绘（单节 cypher 已带回），对每个 `stand.status≠11`：
...
**单节聚焦严禁**：枚举其他节、调 `chapter-publisher`。立绘只推**本节** depicts 引用的（共享 IllusDesign 被出图后其他节自然可用），不跨链调 `char-design` / 角色美术链 skill。铁律（只看 status 不看产物、`-1` 必须重生成覆盖、不读旧提纲/旧剧本/旧图）在单节模式同样适用。汇报只交代目标节（节标题 / 产物链 `ol/sc/vo status` / 本节各 `stand.status` + 本轮是否处理 + 原因），不报其他节。
...
| SecScript（节级定稿） | `chapter-dialoguer` | Skill | -1/0→1→10→11（10 直写，不经 submit） | ✅ 定稿审 |
| LineAudio（节级配音） | `section-voice-publisher` | Skill | -1/0→10→11（10 直写，不经 submit） | — |
| LineAudio（节级逐句音频审） | dashboard 审批中心 | — | 10→11（gate=全部行 approved） | ✅ 逐句审 |
...
- `ch.status` ∈ {-1, 0}（结构未就绪 / 未分节）→ `Skill chapter-structurer <ch_id>`（建 Chapter + 分节 + 建 Section + contains → `ch.status=10`）
...
- `ch.status = 11`（结构已批）→ **遍历各 Section**（按 section_no），逐节判定产物链：
  ...
  - **定稿**（`ol_status=1` 且（无 SecScript ∨ `sc_status ∈ {-1,0,1}`））→ `Skill chapter-dialoguer <sec_id>`（产节级定稿 + 跑 validate + 建 depicts 立绘缺口 → `sc_status=10`）
  - `sc_status = 10` → 等待 dashboard 该节定稿审批（10→11）
  - **配音**（`sc_status = 11` 且（无 LineAudio ∨ `vo_status ∈ {-1,0}`））→ `Skill section-voice-publisher <sec_id>`（节级配音 → `vo_status=10`）
  - `vo_status = 10` → 等待 dashboard 该节逐句音频审批（行级三态全 approved 后节级 10→11）
  - `vo_status = 11` → 该节完成（产物链就绪），继续下一节
- **全部节产物就绪（各节 ol=1 ∧ sc=11 ∧ vo=11）AND `ch.status = 11`** → 检查 depicts 立绘：对每个 `stand.status ≠ 11` 的立绘推进（见下方「立绘委派方式」）
...
**汇报必须逐节点交代**（不得只说「已完成 X」）：列出该章节 Chapter（`ch.status`）+ 各 Section 产物链（`ol/sc/vo status`，标注所处段：提纲/定稿/配音）+ Scene + 立绘 `stand.status`，及本轮是否处理；未推进的说明原因（待审批 / 依赖未满足 / 已完成 / 需用户决策）。**尤其不得把「status=-1 待重做」误报成「节点未创建」**。
...
- **LineAudio（节级配音）**：`0` 待配音 → `10` 声音待审 → `11` 声音已批（completion=11）。`10` 由 section-voice-publisher 直写、不经 submit；驳回归 `0`（重配，不改台词）。
...
**依赖顺序**：`chapter-structurer`（建结构 + 分节 + Section + contains）→ 结构审 `10→11` → 各节 `chapter-outliner`（SecOutline → `ol=1`）→ 各节 `chapter-dialoguer`（SecScript → `sc=10`）→ 各节定稿审 `10→11` → 各节 `section-voice-publisher`（LineAudio → `vo=10`）→ 各节逐句音频审 `10→11` → 推进 depicts 立绘（`char-stand-designer`；上游 IllusDesign≠11 则报警跳过，不跨链）→ 立绘全 `11` = **全章就绪，plot-design 汇报退出**（章级发布 `chapter-publisher` 由用户直接触发：投影合并各节 `台词.jsonl`→`99_game/` 单一章 JSON，行内 audio.key 投影为 say.voice）

**门控**：ch 未到 `11` 不产节级提纲；ol 未到 `1` 不产该节定稿；sc 未到 `11` 不产该节配音、不推该节立绘（避免为未定稿剧本浪费配音/立绘）；**发布不在 plot-design 职责内**——全量推进到全章就绪（ch=11 ∧ 全节 ol=1 ∧ sc=11 ∧ vo=11 ∧ 立绘全 11）即汇报退出，`chapter-publisher` 由用户直接触发。

**节点由 skill 创建**：agent 不直接创建任何图节点或边。Chapter + `has_section` 边 + Section 节点 + `Section-contains->Scene` 边由 `chapter-structurer` 兜底建；`SecOutline` 节点 + `has_outline` 边由 `chapter-outliner` 兜底建；`SecScript` 节点 + `produces` 边 + `depicts` 边 + 立绘缺口节点（`StandingIllustration status=0` + `expands_to`/`ref_style`）由 `chapter-dialoguer` 兜底建（depicts 绑 Scene）；`LineAudio` 节点 + `produces` 边由 `section-voice-publisher` 兜底建；缺口立绘的推进由 plot-design 直调 `char-stand-designer`；IllusDesign 上游由 `char-design` 推进（人工触发，plot-design 不跨链）。

### 4. 审批检查

Chapter 有**结构审**（`10→11`）；SecScript 有**定稿审**（`10→11`）；LineAudio 有**逐句音频审**（行级三态，节级 `10→11`）；StandingIllustration 一道（`10→11`）。（IllusDesign 的审批由 char-design 链管，不在 plot-design 职责内。）
...
- `sc_status = 10` → 定稿待审，等 dashboard（10→11）
- `sc_status = 11` 且（无 LineAudio ∨ `vo_status ∈ {-1,0}`）→ 待配音，调 section-voice-publisher（→10）
- `vo_status = 10` → 逐句音频待审，等 dashboard（行级审 + 节级 10→11）
- `vo_status = 11` → 声音已批，该节产物链就绪

立绘判定：`stand.status = 10` 待审；` = 11` 已批；`< 11` 未就绪需推进——**推进前须确认上游 IllusDesign=11，否则报警跳过**（不跨链调 char-design）。

**只有 `ch.status=11` AND 全部节产物就绪（各节 SecOutline `1` ∧ SecScript `11` ∧ LineAudio `11`）AND 全部 depicts 立绘 `status=11` 才视为章节就绪**（结构已批 + 各节提纲/定稿/配音与所需立绘均就绪）。
...
`chapter-structurer`（skill，章级建结构 + 分节 + 统合 Scene + 建 Section 纯编排容器）· `chapter-outliner`（skill，节级产提纲，兜底建 SecOutline，素材不足时报缺口）· `chapter-dialoguer`（skill，节级产定稿 + 建 depicts 立绘缺口，兜底建 SecScript）· `section-voice-publisher`（skill，节级定稿后逐句配音，兜底建 LineAudio → vo=10）· `char-stand-designer`（skill，按 depicts 引用按需出立绘）

> `chapter-publisher`（章级发布 `25_剧本/`→`99_game/`）由用户直接触发，**不是 plot-design 的调度对象**。
