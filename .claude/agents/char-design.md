...
- 用**无方向变长路径**查询，避免因边方向写反或只走单条上游路径而漏连。一次查询拿到全部节点 + status（Character→StandingIllustration 最深 4 跳，故 `*1..4`）：
...
**调度方式**：用 `Skill` 工具调用对应 skill，参数 `<char_id> [target_status]`（省略 target_status 则推到最终状态）。
